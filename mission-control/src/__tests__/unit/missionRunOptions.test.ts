...
describe('what a learner sees first', () => {
  it('puts the real rover run before the simulation', () => {
    ...
    const runs = buildRunOptions({ youtubeUrl: 'https://youtu.be/dQw4w9WgXcQ', yardId: 'curiosity' });
  ...
  it('still labels a real run from a yard it does not recognise', () => {
    ...
    const runs = buildRunOptions({ youtubeUrl: 'https://youtu.be/dQw4w9WgXcQ', yardId: 'durban-9' });
  ...
  it('falls back to the simulation when no rover has run it yet', () => {
    const runs = buildRunOptions({ yardId: 'curiosity' });
  ...
  it('treats a legacy videoUrl the same as youtubeUrl', () => {
    ...
    const runs = buildRunOptions({ videoUrl: 'https://youtube.com/watch?v=dQw4w9WgXcQ' });
  ...
  it('offers no real run for an unparseable video url', () => {
    ...
    const runs = buildRunOptions({ youtubeUrl: 'not-a-youtube-link' });
