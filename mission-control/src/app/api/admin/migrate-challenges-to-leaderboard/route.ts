...
export async function POST(_request: NextRequest) {
  // Admin only, checked before anything is read. This reads every learner
  // record and rewrites leaderboard entries, and it shipped with no check at
  // all: anyone who knew the URL could run it against the live project.
  try {
    await requireAdmin();
  } catch (error) {
    ...
    if (error instanceof ForbiddenError) {
      return NextResponse.json(
        { success: false, error: 'Only an admin can run this migration' },
        { status: 403 },
      );
    }
    return NextResponse.json(
