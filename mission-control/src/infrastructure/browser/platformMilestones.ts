...
interface Milestones {
  ...
  /** Whether the first-visit welcome has been closed or followed. */
  welcomeSeen: boolean;
}

const EMPTY: Milestones = { visitedRoutes: [], missionCreated: false, welcomeSeen: false };
...
/**
 * Whether this learner has already dealt with the first-visit welcome.
 *
 * Kept with the milestones rather than in a key of its own because it is the
 * same kind of fact - something this learner has done - and it needs the same
 * per-learner keying: on a shared classroom machine the next child should be
 * welcomed too, not skipped because the child before them closed it.
 */
export function hasSeenWelcome(): boolean {
  return readMilestones().welcomeSeen;
}

export function recordWelcomeSeen(): void {
  const current = readMilestones();
  if (current.welcomeSeen) return;
  write({ ...current, welcomeSeen: true });
}
